...
void setup() {
  ...
  background(255);
  ellipseMode(CORNER);
  requestedCellsPerGeneration = 100;
...
class CellularAutomaton {
  ...
  public String[] getRulePatternAsArray() {
    ...
    String rulePatternArray = getRulePatternAsString();
    for (int i = 0; i < rulePatternArray.length(); i++){
      output[i] = rulePatternArray.substring(i, i+1);
    }
    return output;
  }
  ...
  private String getNewCellState(String neighborhood) {
    ...
    if (neighborhood.equals("111")) return arr[0]; 
    else if (neighborhood.equals("110")) return arr[1];
    else if (neighborhood.equals("101")) return arr[2];
    else if (neighborhood.equals("100")) return arr[3];
    else if (neighborhood.equals("011")) return arr[4];
    else if (neighborhood.equals("010")) return arr[5];
    else if (neighborhood.equals("001")) return arr[6];
    else return arr[7];
  }
  ...
  public void show() {
    for (int i = 0; i < cellArray.length; i++) {
      if (cellArray[i].equals("1")) {
        int redNumber = (int) (Math.random() * 120) + 80;
        int blueNumber = (int) (Math.random() * 80) + 140;
        fill(redNumber, 0, blueNumber);
        noStroke();
       // rect(i * getCellSize(), 30 + getCurrentGenerationNumber() * getCellSize(), getDrawSize(), getDrawSize());
       ellipse(i * getCellSize(), 30 + getCurrentGenerationNumber() * getCellSize(), getDrawSize(), getDrawSize());
      }
    }
